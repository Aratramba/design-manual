...
/**
 * Test body html change
 */

test.cb('config: body html change', t => {
  t.plan(1);
  rimraf.sync(config.output);

  const expected = `
    Starting design manual
    - Detected changed configuration (body html)
  ...
  buildAndMatchLogs(null, config, null, () => {
    buildAndMatchLogs(t, Object.assign(config, { bodyHtml: '<script></script>' }), expected);
  });
});