...
/**
 * Test prerender change
 */

test.cb('config: prerender change', t => {
  t.plan(1);
  rimraf.sync(config.output);

  const expected = `
    Starting design manual
    - Detected changed configuration (prerender)
  ...
  buildAndMatchLogs(null, config, null, () => {
    buildAndMatchLogs(t, Object.assign(config, { prerender: 1 }), expected);
  });
});
