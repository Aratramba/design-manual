...
function build(config) {
  ...
  if (options.force) {
    ...
    try {
      let prevConfig = fs.readFileSync(
        path.resolve(options.output, "design-manual-config.json"),
        "utf8"
      );
      if (prevConfig) {
        try {
          ...
          if (
            options.componentHeadHtml !== (prevConfig.componentHeadHtml || "")
          ) {
            log(
              `${changedConfigMessage} (components head html)`,
              options.onLog
            );
            options.forceComponents = true;
          }
          ...
          // force components when component html changed
          if (
            options.componentBodyHtml !== (prevConfig.componentBodyHtml || "")
          ) {
            log(
              `${changedConfigMessage} (components body html)`,
              options.onLog
            );
            options.forceComponents = true;
          }
          ...
          if (
            options.contentsFlag !== (prevConfig.contentsFlag || "contents")
          ) {
...
function render(type, options, cb) {
  ...
  renderer[type].stderr.on("data", data => {
    console.error(`\n${data}`);
  });
