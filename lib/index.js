...
function build(options) {
  ...
  mkdirp(options.output, function(err) {
    ...
    if (options.renderComponents) {
      ...
      componentsProcess.on('message', function(queue) {
        complete(PROCESS_TYPE_COMPONENTS, options.onComplete);

        // generate pages
        if (options.renderPages && (Array.isArray(queue))) {
          pagesProcess = cp.fork(`${__dirname}/pages.js`, [[JSON.stringify(options)], [JSON.stringify(queue)]]);
