...
<!DOCTYPE html>
  ...
  <body style="margin: 0px; padding: 0px;">
    <div style="padding: 10px; border: 1px solid #ddd;" class="js-output">
      {{output}}
    </div>
    {{componentBodyHtml}}
  </body>
...
function createPage(file, navItems, options, pugOptions, cb) {
  ...
  fileStream.on('data', function(data) {
    renderPage(data, file, navItems, options, pugOptions, cb);
  });
}

function renderPage(data, file, navItems, options, pugOptions, cb) {
  var htmlFile = path.join(options.output, getOutputFile(file));
...
function createComponents(file, navItems, options, cb) {
  async.series([
    ...
    if (results.length) {
      ...
      // and aggregate sections/components pug context
      var mdFile = path.resolve(options.pages, file);
      var fileStream = fs.createReadStream(mdFile);
      fileStream.on('data', function(data) {
