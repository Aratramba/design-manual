...
function createComponents(file, navItems, options, cb) {
  async.series([
    ...
    if (results.length) {
      ...
      var components = JSON.parse(results[1]);
      var i = 0;
      ...
      for(; i < l; i++) {
        ...
        components[i].libFile = generateLibFile(components[i], options, results[0].join(''), libPath);
      }
...
function generateLibFile(component, options, websiteCss, libPath) {
  ...
  // return file name
  return ['./lib/', path.basename(componentPath)].join('');
}
