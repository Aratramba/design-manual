...
<!DOCTYPE html>
  <html>
  <head>
    <script src='https://cdnjs.cloudflare.com/ajax/libs/iframe-resizer/3.5.0/iframeResizer.contentWindow.min.js'></script>
    ...
    if (results.length) {
      ...
      for(; i < l; i++) {
        ...
        html = LIB_TEMPLATE
          ...
          .replace('{{websiteCss}}', results[0].join(''));
        
        id = shortid.generate();
        componentPath = path.resolve(libPath, id + '.html');
        fs.writeFile(componentPath, html);
        components[i].libFile = './lib/' + path.basename(componentPath);
      }
