...
function createPage(file, navItems, options, jadeOptions, cb) {
  ...
  fileStream.on('data', function(data) {
    renderPage(data, file, navItems, options, jadeOptions, cb);
  });
}

function renderPage(data, file, navItems, options, jadeOptions, cb) {
  var htmlFile = path.join(options.output, getOutputFile(file));
...
function createComponents(file, navItems, options, cb) {
  async.series([
    ...
    if (results.length) {
      ...
      // read source md file, find tables of contents 
      // and aggregate sections/components jade context
      var mdFile = path.resolve(options.pages, file);
      var fileStream = fs.createReadStream(mdFile);
      fileStream.on('data', function(data) {
        ...
        for (i = 0, l = tokens.length; i < l; i++) {
          ...
          if (isScanningList) {
            ...
            if (tokens[i].type === 'list_end') {
              ...
              var html = jade.renderFile(__dirname +'/../template/templates/_components.jade', { 
        ...
        renderPage(data, file, navItems, options, {}, cb);
      });
    }
  });
}
