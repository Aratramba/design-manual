...
function readFiles(options) {
  recursive(options.pages, ['!*.md'], function (err, files) {
    ...
    if (!files) {
      throw new Error('No .md files found inside ' + options.pages);
    }
...
function processFiles(results, options, navItems, cb) {
  async.each(results, function(result, next) {
    if (path.basename(result.file) !== options.componentsPage) {
...
function renderComponent(file, options, cb) {
  ...
  var child = spawn(electron, [__dirname + '/electron.js', 'http://localhost:' + options.prerender.port + '/' + options.prerender.path +  file], {
    stdio: ['ipc'],
...
function getNavigation(files, options) {
  ...
  return files.map(function(file) {
    if (path.relative(options.pages, file).indexOf('/') === -1) {
      return {
