  ...
  fs.readdir(options.pages, function(err, files){
    ...
    files.forEach(function(filename){
      ...
      switch(filename){
        ...
        createComponents(filepath, filename, options);
        break;

        case PAGE_INDEX:
        createIndex(filepath, filename, options);
        break;

        default:
        createPage(filepath, filename, {}, options);
        break;
      }
    });
  });
...
function createPage(filepath, filename, jadeOptions, options, cb){
...
function createComponents(filepath, filename, options){
  ...
  async.each(options.includeCss, function(file, cb){
    ...
    createPage(filepath, filename, jadeOptions, options);
  });
}
...
function createIndex(filepath, filename, options){
  createPage(filepath, filename, {}, options);
}
