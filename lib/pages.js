...
if (options.renderComponents) {
  try {
    ...
    console.log('Design Manual json not found at ' + options.components + '. Pages are rendered without components. Please try again.');
    process.send(0);
    // return;
  }
}
...
function ignoreNewer(mdFile, stats) {
  ...
  return (new Date(stats.mtime) < new Date(htmlStat.mtime));
}
...
recursive(options.pages, ['!*.md', ignoreNewer], function (err, files) {
