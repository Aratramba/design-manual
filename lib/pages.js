      ...
      if (tokens[i].type === 'paragraph' && regex.test(tokens[i].text.trim())) {
        ...
        tokens[i].text = tokens[i].text.replace(/!{(.*)}/g, (match, componentName) => {
          ...
          if (componentData) {
            ...
            componentData.meta.browsers = options.componentMeta.browsers.map(browser => {
              return Object.assign({}, browser, { active: ((componentData.meta.browsers || []).indexOf(browser.name) > -1) });
            });
            ...
            componentHTML = pug.renderFile(COMPONENT_TEMPLATE, {
              component: {
                ...
                meta: {
                  ...
                  browsers: componentData.meta.browsers
                },
