...
function ignore(mdFile, stats) {
  ...
  if (mdFileData.indexOf(`!{...autofill}`) > -1 || mdFileData.indexOf(`\${...autofill}`) > -1) {
    return false;
  }
    ...
    try {
      ...
      for (let i = 0, l = tokens.length; i < l; i++) {
        ...
        if (tokens[i].type === 'paragraph' && COMPONENT_EMBED_REGEX.test(tokens[i].text.trim())) {
          const matches = tokens[i].text.trim().match(COMPONENT_EMBED_REGEX);
      ...
      for (let i = 0, l = pageComponents.length; i < l; ++i) {
        ...
        tokens[pageComponents[i].index].text = tokens[pageComponents[i].index].text.replace(COMPONENT_EMBED_REGEX, (match, prefix, componentName) => {
