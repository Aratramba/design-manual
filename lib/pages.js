...
const COMPONENT_EMBED_REGEX = /!!?{(.*)}/g;
const COMPONENT_TYPE_REGULAR = 'regular';
const COMPONENT_TYPE_FRAMELESS = 'frameless';
  ...
  fileStream.on('data', (data) => {    
    ...
    for (let i = 0, l = tokens.length; i < l; i++) {
      ...
      if (tokens[i].type === 'paragraph' && COMPONENT_EMBED_REGEX.test(tokens[i].text.trim())) {
        ...
        tokens[i].text = tokens[i].text.replace(COMPONENT_EMBED_REGEX, (match, componentName) => {
          ...
          let embedType = COMPONENT_TYPE_REGULAR;
          if (match.substr(0, 2) === '!!') {
            embedType = COMPONENT_TYPE_FRAMELESS;
          }
