  ...
  fs.readdir(options.pages, function(err, files){
    ...
    files.forEach(function(filename){
      ...
      switch(filename){
        ...
        createComponents(filepath, filename);
        break;

        case PAGE_INDEX:
        createIndex(filepath, filename);
        break;

        default:
        createPage(filepath, filename, {});
        break;
      }
    });
  });
...
function prepareNavigation(files){
  ...
  navItems = files.map(function(file){
    return {
...
function getPageTitle(filename){
  return [getPageName(filename), options.meta.title, options.meta.domain].join(' - ');
}
...
function createPage(filepath, filename, jadeOptions){
  ...
  fileStream.on('data', function(data){
    ...
    var locals = assign({
      ...
      navItems: navItems,
      pageTitle: getPageTitle(filename),
...
function createComponents(filepath, filename){
  ...
  async.each(options.includeCss, function(file, cb){
    ...
    createPage(filepath, filename, jadeOptions);
  });
}
...
function createIndex(filepath, filename){
  createPage(filepath, filename, {});
}
