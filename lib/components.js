  ...
  fs.stat(options.components, function(err, stats) {
    ...
    fs.readFile(path.resolve(options.components), function(err, componentsJSON) {
      ...
      fs.readFile(path.resolve(options.output, 'design-manual.json'), function(err, designManualJSON) {
        ...
        for (i = 0, l = componentsJSON.length; i < l; ++i) {
          ...
          // matches found
            ...
            var clone = assign({}, designManualJSON[i]);
            delete clone.dm;
        ...
        if (queue.length === 1) {
          ...
          console.log(`- Found ${queue.length} changed component`);
        }

        if (!queue.length) {
          return done(options, designManualJSON);
        }
...
function done(options, designManualJSON) {
