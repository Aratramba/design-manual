  ...
  fs.stat(options.components, (err, stats) => {
    ...
    fs.readFile(path.resolve(options.components), (err, componentsJSON) => {
      ...
      fs.readFile(
        ...
        (err, designManualJSON) => {
          if (err) {
            // no previous components found
          }

          try {
            designManualJSON = JSON.parse(
              (designManualJSON || "[]").toString()
            );
          } catch (err) {
            designManualJSON = [];
          }
          ...
          for (let i = 0, l = componentsJSON.length; i < l; ++i) {
            // check for meta/name, if not present ignore this component
            if (
              !componentsJSON[i].meta ||
              typeof componentsJSON[i].meta.name === "undefined"
            ) {
              continue;
            }

            // parse description using markdown so it equals the final output
            if (componentsJSON[i].meta && componentsJSON[i].meta.description) {
              componentsJSON[i].meta.description = marked(
                componentsJSON[i].meta.description
              );
            }
          ...
          componentsJSON = componentsJSON.filter(item => {
...
function render(options, components, designManualJSON, queue) {
  ...
  function nextComponent(components, i) {
...
function generateLibFile(component) {
  ...
  const html = LIB_TEMPLATE.replace("{{output}}", component.output)
    ...
    .replace(
      "{{title}}",
      [component.meta.name, options.meta.title, options.meta.domain].join(" - ")
    );
      ...
      (err, data) => {
        ...
        if (files) {
          for (let i = 0, l = files.length; i < l; ++i) {
            ...
            if (!match.length) {
              rimraf.sync(path.resolve(options.output, "lib", file), {
                read: false
              });
            }
          }
