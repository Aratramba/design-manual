...
function createOutput(options, cb) {
  mkdirp(options.output, function(err) {
    if (err) {
      throw err;
    }

    checkJSON(options, cb);
  });
}
...
function checkJSON(options, cb) {
  fs.stat(options.components, function(err, stats) {
    ...
    fs.readFile(path.resolve(options.components), function(err, data) {
      ...
      render(options, data.toString(), cb);
    });
  });
}
...
function render(options, componentsJSON, cb) {
