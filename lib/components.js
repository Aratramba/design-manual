...
function checkJSON(options) {
  fs.stat(options.components, function(err, stats) {
    ...
    fs.readFile(path.resolve(options.components), function(err, componentsData) {
      if(err) { 
        throw err;
      }
      ...
      fs.readFile(path.resolve(options.output, 'design-manual.json'), function(err, designManualData) {
        
        // remove design manual object from design-manual data
        designManualData = JSON.parse(designManualData.toString());
        for (var i = 0, l = designManualData.length; i < l; ++i) {
          delete designManualData[i].dm;
        }

        // do deepequal check of components vs. design-manual
        if (deepEqual(JSON.parse(componentsData.toString()), designManualData)) {
          ...
          // start render
          render(options, componentsData.toString());
        }
      });
    });
  });
...
function render(options, componentsJSON) {
