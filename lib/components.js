...
<!DOCTYPE html>
  ...
  <body style="margin: 0px; padding: 0px;">
    ...
    {{componentBodyHtml}}
<pre style="display: none;" class="dm-raw-source">
{{raw_output}}
</pre>
  </body>
</html>
  ...
  fs.stat(options.components, function(err, stats) {
    ...
    fs.readFile(path.resolve(options.components), function(err, componentsJSON) {
      ...
      fs.readFile(path.resolve(options.output, 'design-manual.json'), function(err, designManualJSON) {
        ...
        designManualJSON = JSON.parse(designManualJSON.toString()).sort(compareJSON);
        componentsJSON = JSON.parse(componentsJSON.toString()).sort(compareJSON);
        
        // remove design manual object from design-manual data
        for (var i = 0, l = designManualJSON.length; i < l; ++i) {
          delete designManualJSON[i].dm;
        }
        ...
        // do deepequal check of components vs. design-manual
        if (deepEqual(componentsJSON, designManualJSON)) {

          // shortcut
          console.log('- Found no changed components');
          done();

        } else {

          // start render
          startServer(options, componentsJSON);
        }
      });
    });
  });
...
function startServer(options, componentsJSON) {
  ...
  createOutput(options, componentsJSON);
}
...
function createOutput(options, componentsJSON) {
...
function render(options, components) {
  ...
  function nextComponent(i) {
    ...
    if (!components.length) {
      return fs.writeFile(path.resolve(options.output, 'design-manual.json'), '[]', function() {
        done();
      });
    }
    ...
    components[i].dm.libFile = generateLibFile(components[i], options, libPath);
    ...
    renderComponent(components[i].dm.libFile, options, function(result) {
      ...
      if (i < components.length - 1) {
        ...
        fs.writeFile(path.resolve(options.output, 'design-manual.json'), JSON.stringify(components), function() {
          done();
        });
      }
    });
